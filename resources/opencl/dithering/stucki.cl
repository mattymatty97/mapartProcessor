...
const static int4 Stucki_param[12] = {(int4)(0, 1, 8, 42), (int4)(0, 2, 4, 42),
 (int4)(1, -2, 2, 42), (int4)(1, -1, 4, 42), (int4)(1, 0, 8, 42), (int4)(1, 1, 4, 42), (int4)(1, 2, 2, 42),
 (int4)(2, -2, 1, 42), (int4)(2, -1, 2, 42), (int4)(2, 0, 4, 42), (int4)(2, 1, 2, 42), (int4)(2, 2, 1, 42)};

__kernel void Stucki (
      ...
      for (int x = 1; x < (width - 1); x++)  // Each work item processes a line (ignoring 1st and last pixels)...
      {
          ...
          */
            ...
            for (uchar j = 0; j < 12; j++){
                ...
                if (( x + Stucki_param[j][1] ) >= 0 &&  ( x + Stucki_param[j][1]) < width && ( y + Stucki_param[j][0]) >= 0 && ( y + Stucki_param[j][0]) < height){
                    error_index =  (width * ( y + Stucki_param[j][0])) + x + Stucki_param[j][1];
                    dst_error   =  vload4(error_index, err_buf);
                    dst_error   += (min_d * Stucki_param[j][2] / Stucki_param[j][3]);
                    vstore4(dst_error, error_index, err_buf);
                }
            }
